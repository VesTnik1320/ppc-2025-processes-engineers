...
### Результаты производительности (задача: ring_topology)

#### Время выполнения `pipeline` (секунды)

| Процессы |   Время    | Ускорение | Эффективность |
|----------|------------|-----------|---------------|
| 1        | 0.00025678 | 1.000     | 1.000         |
| 2        | 0.00333172 | 0.077     | 0.039         |
| 3        | 0.00472360 | 0.054     | 0.018         |
| 4        | 0.00735162 | 0.035     | 0.009         |
| 5        | 0.00914614 | 0.028     | 0.006         |
| 6        | 0.01033436 | 0.025     | 0.004         |
| 7        | 0.01066848 | 0.024     | 0.003         |
| 8        | 0.01105108 | 0.023     | 0.003         |
| 16       | 0.04083082 | 0.006     | 0.0004        |

#### Время выполнения `task_run` (секунды)

| Процессы |   Время    | Ускорение | Эффективность |
|----------|------------|-----------|---------------|
| 1        | 0.00032392 | 1.000     | 1.000         |
| 2        | 0.00263452 | 0.123     | 0.062         |
| 3        | 0.00455500 | 0.071     | 0.024         |
| 4        | 0.00633040 | 0.051     | 0.013         |
| 5        | 0.00795116 | 0.041     | 0.008         |
| 6        | 0.00805768 | 0.040     | 0.007         |
| 7        | 0.00938996 | 0.035     | 0.005         |
| 8        | 0.00995994 | 0.033     | 0.004         |
| 16       | 0.03822198 | 0.008     | 0.0005        |

### Анализ результатов

#### Основные наблюдения:
1. **Наилучшая производительность при 1 процессе:** что ожидаемо для коммуникационно-насыщенной задачи, где вычислительная работа минимальна и отсутствуют накладные расходы на передачу данных
2. **Рост времени выполнения с увеличением количества процессов:** обусловлен накладными расходами на MPI-коммуникации и последовательной природой передачи в кольцевой топологии
3. **Нелинейная зависимость времени от числа процессов:**
   - При 2 процессах время увеличивается в ~13 раз по сравнению с 1 процессом
   - При 16 процессах время увеличивается в ~159 раз по сравнению с 1 процессом
4. **Сравнение двух режимов выполнения:**
   - Режим `task_run` демонстрирует лучшее время на 1 процессе (0.00032с против 0.00026с)
   - При большом числе процессов оба режима показывают сопоставимые результаты
5. **Крайне низкая эффективность параллелизации:** эффективность падает с 100% на 1 процессе до менее 0.1% на 16 процессах
